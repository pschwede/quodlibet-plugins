...
class QLScrobbler(EventPlugin):
    ...
    def __init__(self):
        ...
        self.nonowpl = config_get('nonowpl')
        log("Sending now playing: "+self.nonowpl)
        self.queue.nonowpl = self.nonowpl == "true"
    ...
    def plugin_on_song_started(self, song):
        ...
        self.elapsed = 0
        if not self.nonowpl and self.__enabled and not player.playlist.paused and not parse.Query(self.exclude).search(song):
    ...
    def plugin_on_unpaused(self):
        self.unpaused_time = time.time()
        if self.nonowpl and self.__enabled and self.nowplaying and not parse.Query(self.exclude).search(song):
    ...
    def PluginPreferences(self, parent):
        ...
        nonowpl = ConfigCheckButton(_("Don't submit 'Now playing'"),
                                'plugins', 'scrobbler_nonowpl')
        nonowpl.set_active(config_get('nonowpl') == "true")
        table.attach(nonowpl, 0, 2, 7, 8)
